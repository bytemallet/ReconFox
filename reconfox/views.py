...
import reconfox.tool.retriever_modules.dorks as dorks
...
def add_domain(request):
    ...
    if Domain.objects.filter(domain=domain_name).exists():
        ...
        d = Domain.objects.create(domain=domain_name,email_domain=email_domain,full_passive=full_passive)
        trio.run(reconfox.utils.init, d.id)
...
def domain_info(request, domain_id):
    ...
    if domain.whois_data or domain.dns_records:
        data = {'whois': domain.whois_data, 
                'dns': domain.dns_records, 
                'email_domain': domain.email_domain,
                'email_pattern': domain.email_pattern}
    else:
...
def people_all(request):
    ...
    for entry in people_data.iterator():
        print(entry.job_title)
        p = {}
        p["id"] = entry.id
        p["name"] = entry.name
        p["phones"] = len(entry.phones)
        p["accounts"] = 0
        p["job_title"] = entry.job_title
...
def get_available_tasks(request):
    reconfox.utils.load_tasks(request.GET['domain_id'])
    reconfox.utils.load_custom_tasks(request.GET['domain_id'])
    tasks = []
...
def execute_task(request):
    ...
    if not info.custom:
        t = getattr(reconfox.tasks, tid)
    task = None
    if info.celery_id:
        result = AsyncResult(info.celery_id)
        if result.state != "PENDING":
            if info.custom:
                domain_path = "reconfox.tool.custom_modules." + info.name
                task = reconfox.tasks.executeCustomTask.delay(domain.id,domain_path)
            else:
